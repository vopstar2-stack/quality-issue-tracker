...
import { estimateCountermeasure } from "@/lib/anthropic";
...
export async function POST(
  ...
  const countermeasure =
    typeof body?.countermeasure === "string" ? body.countermeasure : issue.countermeasure;

  const photos = await listPhotos(id);

  try {
    const text = await estimateCountermeasure({
      description,
      cause,
      countermeasure,
